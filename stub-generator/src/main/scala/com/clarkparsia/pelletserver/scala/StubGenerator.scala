...
object StubGenerator {
  ...
  val PACKAGE = "com.clarkparsia.pelletserver.scala.api"
  val TOP_CLASS = PelletServer.WebResource
  
  val schema = ModelFactory.createDefaultModel
  var outputDirectory = new File("target/generated-sources/ps")
...
//	 	      }))
  ...
  private def findMethods(jsonServiceDescription: URL) = {
	  // TODO: Need to GET the URL with header "Accept: text/json"
	  val json = parse(new InputStreamReader(jsonServiceDescription.openStream))
